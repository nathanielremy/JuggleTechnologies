...
class SignupVC: UIViewController, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    fileprivate func createUser(fromtextFields textFields: (email: String, password: String, firstName: String, lastName: String)) {
        Auth.auth().createUser(withEmail: textFields.email, password: textFields.password) { (newUser, err) in
            ...
            storageRef.putData(imageData, metadata: nil, completion: { (metaData, err) in
                ...
                storageRef.downloadURL(completion: { (link, err) in
                    if let error = err {
                        print("Error fetching profileImageURL from storage: \(error)")
                        DispatchQueue.main.async {
                            self.disableAndAnimate(false)
                        }
                        return
                    ...
                    guard let profileImageURLString = link?.absoluteString else {
                    ...
                    let ref = Database.database().reference().child(Constants.FirebaseDatabase.usersRef)
    ...
    fileprivate func approveProfileImage() -> UIImage {
        ...
        guard let plusPhotoImage = self.plusPhotoButton.imageView?.image else {
            image = #imageLiteral(resourceName: "DefaultProfileImage")
            return image
        }
        
        if plusPhotoImage == #imageLiteral(resourceName: "PlusPhoto") {
            image = #imageLiteral(resourceName: "DefaultProfileImage")
            return image
        } else {
